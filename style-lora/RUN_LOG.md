...
|---|---|---|---|---|---|---|---|---|
| 2026-08-27 | Together.ai (job_id=ft-59fe1f00-367d) | Qwen/Qwen3.5-9B | 16 | 32 | 3 | 책 2권(꽃은 미끼야+메리싸이코) v1, raw text 3000자 청크 | 다운로드 됨 (대시보드 Checkpoints 탭, SDK 함수명은 안 맞았음) | 실패 — 짧은 프롬프트로 테스트하니 몇 줄 후 헛소리/반복으로 무너짐. 원인은 모델이 아니라 학습 데이터 형식(긴 덩어리 이어쓰기로만 학습되어 짧은 프롬프트 분포에 안 맞음) |

\* 코퍼스 버전: `data/raw/`에 넣은 책이 바뀌면(신작 추가 등) train_corpus.txt도 달라지므로,
간단히 "책 2권(꽃은 미끼야+메리싸이코) v1" 식으로 구분 기록.

## 교훈: 짧은 프롬프트 테스트가 계속 무너지면 모델을 의심하기 전에

1. **JSONL 실제 내용을 먼저 눈으로 확인** — 문단이 깨끗하게 이어지는지, 책 경계에서
   엉뚱하게 섞이진 않았는지
2. **학습 형식이 실제 사용 방식과 맞는지 확인** — `train_together.py`가 이제
   raw text 청크 대신 `{"prompt": 앞부분, "completion": 뒷부분}` 쌍으로 학습시키도록
   바뀜. 짧은 프롬프트로 이어쓰기를 시키려면 학습도 짧은 프롬프트 기준으로 시켜야 함
3. 테스트할 때 프롬프트를 한두 문장이 아니라 **8~10줄 정도의 장면**으로 줘서
   학습 시 봤던 분포와 비슷하게 맞추기
4. 이 세 개를 다 맞춘 뒤에도 무너지면, 그때 베이스 모델 자체를 의심 (예: Qwen3-8B로 교체)
